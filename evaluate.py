...
def get_arguments():
    ...
    parser.add_argument("--real_mask_dir", type=str, required=True,
                        help="Path of the directory containing the ground-truth masks.")
    parser.add_argument("--predicted_mask_dir", type=str, default=None,
                        help="Path of the directory containing the predicted masks.")

    parser.add_argument("--in_2d", action="store_true", default=False,
                        help="If issued, will perform evaluation in 2D even if the mask is 3D.")
...
def run():
    ...
    assert os.path.isdir(
        args.real_mask_dir), f"The directory of ground truth masks {args.real_mask_dir} does not exist."
    assert os.path.isdir(
        args.predicted_mask_dir), f"The directory of predicted masks {args.predicted_mask_dir} does not exist."
