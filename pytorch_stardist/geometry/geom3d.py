...
def polyhedron_to_label(dist, points, rays, shape, prob=None, thr=-np.inf, labels=None, mode="full", verbose=True,
    ...
    if len(ind) == 0:
        if verbose:
            print("warning: no points found with probability>= {thr:.4f} (returning background-only image)".format(
                thr=thr))
        return lbl
...
def dist_to_volume(dist, rays):
    ...
    dist.shape[-1] == len(rays) or _raise(
        ValueError("dist.shape[-1] = {d} but should be {l}".format(d=dist.shape[-1], l=len(rays))))
...
def export_to_obj_file3D(polys, fname=None, scale=1, single_mesh=True, uv_map=False, name="poly"):
    ...
    try:
        ...
        print(e)
        raise ValueError(
            "polys should be a dict with keys 'dist', 'points', 'rays_vertices', 'rays_faces'  (such as generated by StarDist3D.predict_instances) ")
