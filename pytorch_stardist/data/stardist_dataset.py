...
def get_train_val_dataloaders(opt, rays=None, train_augmenter=None):
    ...
    if (Path(opt.data_dir) / "val").exists():
        ...
        warnings.warn(
            f"didn't find the validation folder <{Path(opt.data_dir) / 'val'}>. Splitting data from {Path(opt.data_dir) / 'train'} with val_size={opt.val_size}")
        image_paths_train, mask_paths_train, image_paths_val, mask_paths_val = train_val_split(image_paths_train,
                                                                                               mask_paths_train,
