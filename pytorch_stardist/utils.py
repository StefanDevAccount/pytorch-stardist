...
from .rays3d import Rays_GoldenSpiral
...
try:
    ...
    try:
        ...
        warnings.warn(
            f"Invalid value ({_edt_parallel}) for STARDIST_EDT_NUM_THREADS. Using default value ({_edt_parallel_default}) instead.")
...
def _normalize_grid(grid, n):
    try:
        ...
        raise ValueError(
            "grid = {grid} must be a list/tuple of length {n} with values that are power of 2".format(grid=grid, n=n))
...
def optimize_threshold(Y, Yhat, model, nms_thresh, measure='accuracy', iou_threshs=[0.3, 0.5, 0.7], bracket=None,
                       tol=1e-2, maxiter=20, verbose=1):
    ...
    with tqdm(total=maxiter, disable=(verbose != 1), desc="NMS threshold = %g" % nms_thresh) as progress:
        ...
        def fn(thr):
            ...
            if value is None:
                Y_instances = [model._instances_from_prediction(y.shape, *prob_dist, prob_thresh=prob_thresh,
                                                                nms_thresh=nms_thresh)[0] for y, prob_dist in
                               zip(Y, Yhat)]
            ...
            if verbose > 1:
                ...
                progress.set_postfix_str(
                    "{prob_thresh:.3f} -> {value:.3f}".format(prob_thresh=prob_thresh, value=value))
                progress.refresh()
            return -value
...
def mask_to_categorical(y, n_classes, classes, return_cls_dict=False):
    ...
    if not set(y_labels).issubset(set(classes.keys())):
        raise ValueError(
            f"all gt labels should be present in class dict provided \ngt_labels found\n{set(y_labels)}\nclass dict labels provided\n{set(classes.keys())}")
...
def prepare_conf(opt: ConfigBase):
    ...
    """
    from .data.stardist_dataset import get_train_val_dataloaders
...
def plot_img_label(img, lbl, img_title="image (XY slice)", lbl_title="label (XY slice)", z=None, **kwargs):
    ...
    from .plot.plot import random_label_cmap
