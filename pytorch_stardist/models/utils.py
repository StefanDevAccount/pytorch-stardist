...
def get_scheduler(optimizer, opt, init_lr=None):
    ...
    if opt.lr_policy == "none":
        ...
        def lambda_rule(epoch):
            ...
            if lr_l <= 0:
                warnings.warn(
                    "learning rate negative. current epoch number is higher than the initially total epopchs setted.")
            return lr_l
        ...
        scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=opt.lr_plateau_factor,
                                                   threshold=opt.lr_plateau_threshold, patience=opt.lr_plateau_patience,
                                                   min_lr=opt.min_lr)
...
class Block3D:
    ...
    def set_target_patch(self, idx, patch):
        ...
        self._target_volume[..., src_tgt_slices[0], src_tgt_slices[1], src_tgt_slices[2]] = patch[
            ..., tgt_slices[0], tgt_slices[1], tgt_slices[2]]
        ...
        for i in range(len(x)):
            for j in range(len(y)):
                for k in range(len(z)):
                    ...
                    src_target_z, src_target_x, src_target_y = ((src_patch_start % self.grid) != 0).astype(
                        int) + src_patch_start // self.grid
