...
class Resnet(nn.Module):
    def __init__(
        ...
        assert (np.log2(
            n_downs) <= n_blocks).all(), f" n_downs > n_blocks. The number of resnet blocks (n_blocks={n_blocks}) does not allow to perform n_downs={n_downs} downsampling. Set n_downs<=n_blocks."
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_and_residual_block(self, n_filter_in, n_filter_out, kernel_size, stride, n_conv_per_block,
                                      batch_norm, activation, last_conv_bias_if_batch_norm):
        n_dim = len(kernel_size)
        kernel_size = np.array(kernel_size)
        pad_size = list(zip(kernel_size[::-1] // 2, (kernel_size - 1)[::-1] // 2))
        pad_size = itertools.chain(*pad_size)
        ...
        # First conv
        conv_block += [padding, conv_layer(n_filter_in, n_filter_out, kernel_size=kernel_size, stride=stride,
                                           bias=not batch_norm)]
        if batch_norm:
        ...
        for _ in range(n_conv_per_block - 2):
            conv_block += [padding,
                           conv_layer(n_filter_out, n_filter_out, kernel_size=kernel_size, bias=not batch_norm)]
            if batch_norm:
                conv_block += [norm_layer]
            conv_block += [activation]
        ...
        if any(s != 1 for s in stride) or n_filter_in != n_filter_out:
            ...
            residual_block = [padding, conv_layer(n_filter_in, n_filter_out, kernel_size=kernel_size, stride=stride,
                                                  bias=not batch_norm)]
        else:
