...
def sample_patches(datas, patch_size, n_samples, valid_inds=None, verbose=False):
    ...
    if valid_inds is None:
        valid_inds = tuple(_s.ravel() for _s in np.meshgrid(
            *tuple(np.arange(p // 2, s - p // 2 + 1) for s, p in zip(datas[0].shape, patch_size))))
...
def get_valid_inds(img, patch_size, patch_filter=None):
    ...
    if not all((0 < s <= d for s, d in zip(patch_size, img.shape))):
        raise ValueError("patch_size %s negative or larger than image shape %s along some dimensions" % (
        str(patch_size), str(img.shape)))

    if patch_filter is None:
        # only cut border indices (which is faster)
        patch_mask = np.ones(img.shape, dtype=bool)
        valid_inds = tuple(
            np.arange(p // 2, s - p + p // 2 + 1).astype(np.uint32) for p, s in zip(patch_size, img.shape))
