...
def get_arguments():
    ...
    parser.add_argument("--make_image_binary", action="store_true", default=False,
                        help="Convert the image to binary image. Used to connect object in 3D mask of 2d instances.")
    ...
    parser.add_argument("--object_median_size", type=float, nargs='+', default=[],
                        help="Estimation of the median size of object in the images in space separated format: [DEPTH] HIGH WIDTH ; ex: `15 15` in 2D and `8 15 15` in 3D . If given, will rescale the images so that the median object size is the same as the one in the dataset used to train the model.")
    parser.add_argument("--inference_patch_size", type=float, nargs='+', default=None,
                        help="If given, will do inference on patches of size `patch_size` to reduce memory usage; The result will be the same as the one obtained by doing inference on the whole image.")
    parser.add_argument("--use_gpu", action="store_true", default=False, help="Whether to use GPU.")
    parser.add_argument("--use_amp", action="store_true", default=False,
                        help="Whether to use Automatic Mixed Precision.")

    parser.add_argument("--results_dir", type=str, default=r"./results",
                        help="Path of the directory containing the images.")

    return parser.parse_args()


def predict(model: StarDistBase, image_paths, object_median_size=None, normalize_channel="independently",
            patch_size=None, make_image_binary=False):
    ...
    for image_path in tqdm(image_paths):
        ...
        if image.ndim == 4 and not model_is_3d:
            ...
            depth = image.shape[1]
            pred_mask = np.stack([model.predict_instance(image[:, d], patch_size=patch_size)[0] for d in range(depth)],
                                 axis=0)
        ...
        if scale is not None:
            ...
            pred_mask = resize_volume(pred_mask.astype("float32"), target_size=tuple(orig_size),
                                      method="nearest").astype(pred_mask.dtype)
...
def run(predict=predict):
    ...
    if tmp_ndim > 0:
        if args.is_3d:
            ...
            warnings.warn(
                f"len(object_median_size)={tmp_ndim} whereas model is `is_3d`=False. If the image is in 3D, the inference will be performed independently on each slice.")
    else:
    ...
    for (image_path, image, mask) in predict(model, image_paths, object_median_size=args.object_median_size,
                                             patch_size=args.inference_patch_size,
                                             make_image_binary=args.make_image_binary):
